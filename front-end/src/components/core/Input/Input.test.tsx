...
describe('<Input />', () => {
  ...
  it('render input element for all types excluding "TextArea"', () => {
    const mountWrapperWithTextType = mountWithTheme(
      <Input {...requiredProps} />,
    );
    const mountWrapperWithEmailType = mountWithTheme(
      <Input {...requiredProps} type={InputTypes.Email} />,
    );
    const mountWrapperWithNumberType = mountWithTheme(
      <Input {...requiredProps} type={InputTypes.Number} />,
    );
    const mountWrapperWithPasswordType = mountWithTheme(
      <Input {...requiredProps} type={InputTypes.Password} />,
    );
    const mountWrapperWithTextAreaType = mountWithTheme(
      <Input {...requiredProps} type={InputTypes.TextArea} />,
    );
    ...
    expect(mountWrapperWithPasswordType.find('input').first().length).toEqual(
      1,
    );

    expect(
      mountWrapperWithTextAreaType.find('textarea').first().length,
    ).toEqual(1);
  });
  ...
  it('sets input\'s placeholder based on "placeholder" prop value', () => {
    const mountWrapper = mountWithTheme(
      <Input {...requiredProps} placeholder={optionalProps.placeholder} />,
    );
  ...
  it('disable input when "disabled" prop set to true', () => {
    const mountWrapper = mountWithTheme(
      <Input {...requiredProps} disabled={optionalProps.disabled} />,
    );
  ...
  it('sets default input value based on "value" prop', () => {
    const mountWrapper = mountWithTheme(
      <Input {...requiredProps} value={optionalProps.value} />,
    );
  ...
  it('renders label if it was set', () => {
    const mountWrapper = mountWithTheme(
      <Input {...requiredProps} label={optionalProps.label} />,
    );
